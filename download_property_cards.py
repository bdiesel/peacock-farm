#!/usr/bin/env python3
"""Download VGSI property cards (PDFs) for all Peacock Farm properties."""
...
CARD_URL = "https://images.vgsi.com/cards/LexingtonMACards//{pid}.pdf"
OUT_DIR = "property_cards"


def lookup_pid(address):
    """Look up the VGSI PID for a given address."""
    # VGSI autocomplete works best with just number + street name, no suffix/city
    # e.g. "358 NORTH EMERSON" not "358 North Emerson Rd, Lexington"
    search_term = address.upper()
    ...
    for suffix in [" RD", " ST", " DR", " WAY", " CIR", " LN", " TER"]:
        if search_term.endswith(suffix):
            ...
            break
    payload = json.dumps({"inVal": search_term, "src": "i_address"}).encode()
...
def download_pdf(pid, out_path):
    """Download property card PDF."""
    url = CARD_URL.format(pid=pid)
    ctx = __import__("ssl").create_default_context()
    ctx.check_hostname = False
    ctx.verify_mode = __import__("ssl").CERT_NONE
...
def main():
    ...
    print(f"Downloading property cards for {total} properties...")
    print(f"Saving to: {os.path.abspath(OUT_DIR)}/\n")

    pid_map = {}  # parcel_id -> pid, saved to JSON for reference

    for i, (addr, parcel) in enumerate(PROPERTIES, 1):
        out_path = os.path.join(OUT_DIR, f"{parcel}.pdf")
        ...
        try:
            ...
            print(f"  [{i:3}/{total}] ERR   {addr} (lookup failed: {e})")
            failed += 1
            time.sleep(0.5)
            continue

        if not pid:
            print(f"  [{i:3}/{total}] ERR   {addr} (no PID found)")
            failed += 1
            continue
        ...
        # Download PDF
        try:
            download_pdf(pid, out_path)
            size_kb = os.path.getsize(out_path) // 1024
            print(f"  [{i:3}/{total}] OK    {addr} → PID {pid} ({size_kb}KB)")
            ok += 1
        except Exception as e:
            print(f"  [{i:3}/{total}] ERR   {addr} → PID {pid} (download failed: {e})")
            failed += 1

        time.sleep(0.15)  # be polite to the server

    # Save PID map
    with open(os.path.join(OUT_DIR, "pid_map.json"), "w") as f:
        json.dump(pid_map, f, indent=2)

    print(f"\nDone. Downloaded: {ok}  Skipped: {skipped}  Failed: {failed}")
    print(f"PID map saved to {OUT_DIR}/pid_map.json")
